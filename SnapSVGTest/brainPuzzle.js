...
var VALID_POINTS = [];		//ËùÓÐÔÚ³õÊ¼»¯Ê±ÓÐÐ§µãµÄ×ø±êË÷Òý	
var BK_GRIDS = []; //±³¾°¾ØÕó
...
var resultArray = []; //ËùÓÐÓÐÐ§·ÅÖÃÎ»ÖÃÁÐ±í
			
/*
Àý1 ÈÆÈÎÒâµãC=£ÛCx Cy£ÝµÄÐý×ª±ä»»¡£Í¼2.28×ÜµÄ±ä»»¿ÉÍ¨¹ýÈý¸ö»ù±¾±ä»»¸´ºÏ¶ø³É¡£ÏÈ½øÐÐÆ½ÒÆ½»»»£¬Æ½ÒÆÁ¿Îª-CxºÍ-Cy£¬È»ºóÈÆÔ­µãÐý×ª¦È½Ç£¬×îºóÔÙ½øÐÐÆ½ÒÆÁ¿ÎªCxºÍCyµÄÆ½ÒÆ±ä»»¡£Òò´Ë£¬ÈÎÒ»µãP¾­¹ýÖð´Î±ä»»ºóµÄÆë´Î×ø±êÎª
...
//³õÊ¼»¯±³¾°¾ØÕó£¨10*10£©£¬²¢¼ÇÂ¼³õÊ¼ÓÐÐ§µãÁÐ±í£¨50¸ö£©
...
//ÔÚÉ¾³ý±»Õ¼ÓÃµÄµãºó£¬ÊÇ·ñ´æÔÚ¹ÂÁ¢¿é£¨Ð¡ÓÚ2£©
function hasSinglePoint(tempGrids, deletePos)
{
	...
	for(var i=0; i<VALID_POINTS.length; i++)
	{
		...
		if(tempGrids[col][row] == INVALID) //Ö»¼ì²éÓÐÐ§µã
			continue;
			
		var neighbours = getPointNeighbourCount(tempGrids, col, row);
					
		if(neighbours.length == 0) //¿éÊýÎª1
			return true;
		else if(neighbours.length == 1) //Ö»ÓÐÒ»¸öÏàÁÚ¿é£¬ÐèÒª½øÒ»²½ÅÐ¶Ï
		{
			var nCol = col;
			var nRow = row;
...
//²âÊÔ·ÅÖÃÊÇ·ñºÏ·¨£¬Èç¹ûºÏ·¨£¬·µ»Ø·ÅÖÃÎ»ÖÃµãµÄÁÐ±í£¬¸ÃÊý¾Ý¿ÉÒÔ×÷Îª¾²Ì¬Êý¾Ý
function testBlock(block, angle, mirrorX, grids, col, row) //²âÊÔ½«·½¿éÐý×ªangleºó£¬·ÅÖÃÔÚgridµÄCOL,ROW´¦ÊÇ·ñºÏ·¨
{
	var ret = {r:false, o:[]};
	
	//console.log('Test place ' + block.name + ' with angle ' + angle + ' at col:' + col + ', row:' + row);
	var count = block.points.length;
	var org = block.points[0]; //ÒÔµÚÒ»¿éÎªÔ­µã½øÐÐÐý×ª
...
//²âÊÔÃ¿¸ö¿éµÄÓÐÐ§·ÅÖÃÎ»ÖÃ£¬·Ö¾µÏñÁ½¸ö£¬Ðý×ªËÄ¸ö·½Ïò
function testGroupOnGrid(index)
{	
	var validCount = 0;
	var validResult = {blockName:brainPuzzleBlocks[index].name, useFlag: false, result:[]}; //Ãû³Æ£¬ÊÇ·ñÔÚ½â¾ö·½·¨ÖÐÊ¹ÓÃ£¬[{½Ç¶È£¬Î»ÖÃ}]
	...
	for(var i=0; i<2; i++) //¾µÏñ
	{
		if(((brainPuzzleBlocks[index].name == "F") || 
			...
			&& (i == 1)) //F G H I J S²»ÐèÒª¾µÏñ
			continue;
			
		for(var j=0; j<angles.length; j++) //Ðý×ª
		{
...
function isValidPoint(validPoints, point) //¸ÃµãÊÇ·ñÊÇÓÐÐ§µã
{
...
function pointsInUseFlag(usedPoints, newPoints) //ÐÂÔöµÄµãÊÇ·ñÒÑ¾­±»Õ¼ÓÃ
{
...
function findOneSolution()
{
	...
	for(var j=0; j<result.length; j++)
	{
		...
		if(findOneSolution() == false) //µÝ¹éÊ§°Ü£¬»ØÍË
		{
			findFlag = false;
			solutionStack.pop();
